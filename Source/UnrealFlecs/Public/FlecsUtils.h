...
namespace flecs_utils
{
	...
	{
		flecs::world_t* flecs_world = const_cast<flecs::world_t*>(ecs_get_world(entity.world()));
		flecs::entity_t comp_id = flecs::_::type<First>::id(flecs_world);
	...
	flecs_pair_result<const First> get_pair(const flecs::entity& entity)
	{
		...
		if (index != -1)
		{
			...
			return { flecs::entity(flecs_world, second), static_cast<const First*>(r.get_column(index)) };
		}
	...
	flecs_pair_result<First> get_pair_mut(const flecs::entity& entity)
	{
		...
		if (index != -1)
		{
			...
			return { flecs::entity(flecs_world, second), static_cast<First*>(r.get_column(index)) };
		}
...
inline uint32 GetTypeHash(flecs::entity& entity)
{
	return HashCombine(GetTypeHash(static_cast<uint64>(entity)), PointerHash(entity.world()));
}